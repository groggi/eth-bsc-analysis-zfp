...
$a_n y^{(n)}+a_{(n-1)}y^{(n-1)}+\ldots+a_1 y'+a_0 = 0$. Dazu ersetzen wir $y^{(n)}$ durch
$\lambda^n \cdot e^{\lambda x}$. Beispielsweise wird aus $a_2 y'' + a_1 y' + y = 0$ wird
$\a_2 lambda^2 e^{\lambda x} + \a_1 lambda e^{\lambda x} + e^{\lambda x} = 0$.
